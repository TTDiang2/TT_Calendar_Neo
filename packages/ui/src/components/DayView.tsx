...
export function DayView({ monthData, layers, selectedDate: _selectedDate, onSelect, onDoubleClick }: Props) {
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 gap-2">
      {/* 头部：日期 / 农历 / 放假 / 染色状态（iOS 大标题卡：左侧日期徽章 + 右侧元信息） */}
      <div
        className="glass-card rounded-3xl md:rounded-xl md:bg-white md:border md:border-gray-200 md:shadow-none flex items-center gap-3.5 px-4 py-3 md:px-4 md:py-3 flex-shrink-0 cursor-pointer active:opacity-90 transition-opacity"
        onClick={() => onSelect(day.date)}
        onDoubleClick={() => onDoubleClick(day.date)}
      >
        <div
          className={clsx(
            'w-14 h-14 md:w-14 md:h-14 rounded-2xl flex-shrink-0 flex flex-col items-center justify-center text-white shadow-md',
            day.is_today
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5 flex-wrap">
            <span className="text-lg md:text-lg font-bold text-gray-900 tracking-tight">
              {weekday}
        ...
        {/* 手机端新建统一走右下角 FAB（20260917 1.2-3）；桌面保留原按钮（零变化红线） */}
        <button
          onClick={(e) => { e.stopPropagation(); onDoubleClick(day.date) }}
          className="hidden md:flex items-center gap-1 px-3 py-1.5 text-sm bg-pink-500 text-white rounded-lg hover:bg-pink-600 active:bg-pink-600 flex-shrink-0"
        >
          <Plus size={14} /> 新建
        </button>
      </div>

      {/* 主内容：iOS 内嵌分组列表——白色圆角卡片 + 大写字头 + 通栏分隔线 */}
          ...
          <div className="p-3 md:p-4 flex flex-col gap-4">
              ...
              <Section icon={<CalendarClock size={14} />} title="日程" count={schedules.length}>
                {schedules.map((it, idx) => (
                  <div
                    ...
                    className={clsx(
                      'flex items-center gap-3 px-3.5 py-2.5 bg-white md:bg-blue-50/40 md:border md:border-blue-100 md:rounded-lg',
                      idx > 0 && 'border-t border-black/5 md:border-t-0 md:mt-1',
                      idx === 0 && 'rounded-t-2xl md:rounded-lg',
                      idx === schedules.length - 1 && 'rounded-b-2xl md:rounded-lg',
                    )}
                  >
                ...
                {visibleEvents.map((ev, idx) => {
                  ...
                  return (
                    <div
                      key={ev.id ?? ev.title}
                      className={clsx(
                        'flex items-start gap-3 px-3.5 py-2.5 bg-white md:bg-white md:border md:border-gray-200 md:rounded-lg',
                        idx > 0 && 'border-t border-black/5 md:border-t-0 md:mt-1',
                        idx === 0 && 'rounded-t-2xl md:rounded-lg',
                        idx === visibleEvents.length - 1 && 'rounded-b-2xl md:rounded-lg',
                      )}
                    >
            ...
            {(openTodos.length > 0 || doneTodos.length > 0) && (
              <Section icon={<ListTodo size={14} />} title="待办" count={openTodos.length + doneTodos.length}>
                <div className="bg-white md:bg-transparent rounded-2xl md:rounded-none overflow-hidden">
                  {openTodos.map((t, i) => <TodoLine key={t.id} todo={t} divider={i > 0 || doneTodos.length > 0} />)}
                  {doneTodos.map((t, i) => <TodoLine key={t.id} todo={t} done divider={i > 0 || openTodos.length > 0} />)}
                </div>
              </Section>
            )}
          </div>
...
function TodoLine({ todo, done, divider }: { todo: Todo; done?: boolean; divider?: boolean }) {
  const overdue = !done && todo.due_date != null && todo.due_date < todayStr()
  return (
    <div className={clsx('flex items-center gap-3 px-3.5 py-2.5 bg-white md:border md:border-gray-200 md:rounded-lg', divider && 'border-t border-black/5 md:border-t-0 md:mt-1')}>
      <span
        className={clsx(
