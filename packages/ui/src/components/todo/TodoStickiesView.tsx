...
export function TodoStickiesView({ todos, selectedTodoId, onSelect, onOpenNotes }: Props) {
  ...
  return (
    <div
      className="h-full overflow-y-auto p-3 md:p-5 columns-2 md:columns-2 lg:columns-3 2xl:columns-4 gap-3 md:gap-5 [background-image:radial-gradient(#e7e2d8_1px,transparent_1px)] [background-size:22px_22px]"
    >
      {stickies.map(({ todo, color, rotate, tapeRotate }) => {
        ...
        return (
          ...
          >
            ...
            {todo.body && (
              <p className="mt-2 hidden md:block text-[12.5px] text-stone-600/90 leading-relaxed break-words whitespace-pre-wrap">
                {todo.body}
              </p>
            )}
          </div>
