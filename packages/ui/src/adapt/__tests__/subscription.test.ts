 ...
 * 档位，而老端（Python）自建图层是递增编号——早起/早睡/约饭的 sort_order
 * 实测都是 10，在手机端被误判成订阅图层、从图层树里消失。该档位已删除，
 * 本测试钉死：用户自建图层无论 sort_order 多大都不是订阅图层。
 */
...
describe('subscriptionLayerFilter', () => {
  ...
  it('【回归】用户自建图层 sort_order=10 不是订阅图层（老端递增编号，早起/早睡/约饭实测踩雷）', () => {
    const isSub = subscriptionLayerFilter(SUB_NAMES)
